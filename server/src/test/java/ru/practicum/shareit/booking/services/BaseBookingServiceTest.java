...
class BaseBookingServiceTest {
  ...
  void findByIfUserIsNotOwnerAndIsNotBookerTest() {
    ...
    assertThrows(ValidationException.class, () -> { service.findById(1,3);},"Нет сообщения: Booking  доступен только Owner и Booker");
  ...
  void createBookingIfItemNotAvailableTest() {
    ...
    assertThrows(ValidationException.class, () -> {service.create(bookingDto, 1L);}, "Нет сообщения: Item недоступен");
  }
  @Test
  void createBookingOwnerTest() {
    BookingDto  bookingDto = BookingDto.builder()
            ...
            .itemId(3L)
            .build();
    assertThrows(ValidationException.class, () -> {service.create(bookingDto, 2L);}, "Сообщение что владелец не может создать Booking нет");
  }

  @Test
  void deleteTest() {
    service.delete(1L);
    assertThrows(NotFoundException.class, () -> {service.findById(1,1);}, "Сообщения что запись найдена нет");
  }
  ...
  void approveIf_NotWaitingTest() {
    ...
    assertThrows(ValidationException.class, () -> {service.approve(bookingApproveDto, 1L);},"Нет сообщения что Booking  не в статусе Wating");
  }
  @Test
  void approveIf_NotOwnerTest() {
    ...
    assertThrows(ValidationException.class, () -> {service.approve(bookingApproveDto, 3L);},"Нет сообщения что Booking подтверждает не ваделец");
  }
  ...
  void findByBookerIfStatusNotValidTest() {
    assertThrows(ValidatetionConflict.class, () -> {service.findByBookerId(2, "NotValidStutus");},"Нет сообщения: Некорректный статус Booking");
  }

  @Test
  void findByOwnerIfStatusNotValidTest() {
    assertThrows(ValidatetionConflict.class, () -> {service.findByOwnerId(1, "NotValidStutus");},"Нет сообщения: Некорректный статус Booking");
  }
